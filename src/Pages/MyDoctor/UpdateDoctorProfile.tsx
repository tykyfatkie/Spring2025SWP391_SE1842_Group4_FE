      ...
      try {
        ...
        try {
          if (doctorData.metadata) {
            metadataObj = JSON.parse(doctorData.metadata);
          }
        } catch (e) {
  ...
  const handleUpdateProfile = async (values: FormValues) => {
    ...
    try {
      ...
      const profileData = {
        ...
        research: finalValues.research || '',
        languages: finalValues.languages || ''
      };

      console.log("Sending data to API:", profileData);

      // Use PATCH method for updating the doctor profile
      const response = await fetch(`${import.meta.env.VITE_API_ENDPOINT}/doctors/${userId}`, {
        method: 'PUT',
        headers: {
