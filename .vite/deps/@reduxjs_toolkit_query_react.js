...
import { require_react } from './chunk-4SFRHSJ3.js'
import {
  SHOULD_AUTOBATCH,
  applyPatches,
  combineReducers,
  ...
  function middlewareWarning(dispatch) {
    if (true) {
      ...
      if (
        ...
        typeof (returnedValue == null ? void 0 : returnedValue.type) ===
          'string'
      ) {
        throw new Error(
          false
  ...
  const prefetch = (endpointName, arg, options) => (dispatch, getState) => {
    ...
    if (force) {
      ...
      const lastFulfilledTs =
        latestStateValue == null ? void 0 : latestStateValue.fulfilledTimeStamp
      if (!lastFulfilledTs) {
        dispatch(queryAction())
        return
      }
        ...
        .addMatcher(hasRehydrationInfo, (draft, action) => {
          ...
          for (const [key, entry] of Object.entries(queries)) {
            if (
              ...
              (entry == null ? void 0 : entry.status) === 'fulfilled' ||
              (entry == null ? void 0 : entry.status) === 'rejected'
            ) {
              draft[key] = entry
            }
          }
        ...
        .addMatcher(hasRehydrationInfo, (draft, action) => {
          ...
          for (const [key, entry] of Object.entries(mutations)) {
            if (
              // do not rehydrate entries that were currently in flight.
              ((entry == null ? void 0 : entry.status) === 'fulfilled' ||
                (entry == null ? void 0 : entry.status) === 'rejected') && // only rehydrate endpoints that were persisted using a `fixedCacheKey`
        ...
        reducer(draft, action) {
          ...
          for (const { type, id } of providedTags) {
            const subscribedQueries =
              (_a = draft[type] ?? (draft[type] = {}))[
                (_b = id || '__internal_without_id')
              ] ?? (_a[_b] = [])
...
function buildSelectors({
    ...
    return (queryArgs) => {
      ...
      const selectQuerySubstate = (state) => {
        ...
        return (
          ...
          null
            ? void 0
            : _b[serializedArgs]) ?? defaultQuerySubState
        )
      }
  ...
  function buildMutationSelector() {
    return (id) => {
      ...
      const selectMutationSubstate = (state) => {
        ...
        return (
          ((_b =
            (_a = selectInternalState(state)) == null
              ? void 0
              : _a.mutations) == null
            ? void 0
            : _b[mutationId]) ?? defaultMutationSubState
        )
      }
  ...
  function selectCachedArgsForQuery(state, queryName) {
    return Object.values(state[reducerPath].queries)
      .filter(
        (entry) =>
          (entry == null ? void 0 : entry.endpointName) === queryName &&
...
function buildCreateApi(...modules) {
  return function baseCreateApi(options) {
    const extractRehydrationInfo = weakMapMemoize((action) => {
      var _a
      return (_a = options.extractRehydrationInfo) == null
        ? void 0
        : _a.call(options, action, {
...
var buildBatchedActionsHandler = ({ api, queryThunk, internalState }) => {
  ...
  const actuallyMutateSubscriptions = (mutableState, action) => {
    ...
    if (queryThunk.pending.match(action)) {
      ...
      const substate =
        mutableState[(_b = arg.queryCacheKey)] ?? (mutableState[_b] = {})
    ...
    if (queryThunk.rejected.match(action)) {
      ...
      if (condition && arg.subscribe) {
        const substate =
          mutableState[(_c = arg.queryCacheKey)] ?? (mutableState[_c] = {})
        substate[requestId] =
          arg.subscriptionOptions ?? substate[requestId] ?? {}
        mutated = true
      }
  ...
  const isRequestSubscribed = (queryCacheKey, requestId) => {
    ...
    const subscriptions = getSubscriptions()
    return !!((_a =
      subscriptions == null ? void 0 : subscriptions[queryCacheKey]) == null
      ? void 0
      : _a[requestId])
  }
  ...
  const handler = (action, mwApi, internalState2) => {
    ...
    if (canTriggerUnsubscribe(action)) {
      ...
      for (const queryCacheKey of queryCacheKeys) {
        handleUnsubscribe(
          queryCacheKey,
          (_a = state.queries[queryCacheKey]) == null
            ? void 0
            : _a.endpointName,
          mwApi,
          state.config
...
var buildInvalidationByTagsHandler = ({
  ...
  function hasPendingRequests(state) {
    ...
    for (const key in state.queries) {
      if (
        ((_a = state.queries[key]) == null ? void 0 : _a.status) === 'pending'
      )
        return true
    }
    for (const key in state.mutations) {
      if (
        ((_b = state.mutations[key]) == null ? void 0 : _b.status) === 'pending'
      )
        return true
    }
    return false
...
var buildQueryLifecycleHandler = ({
  ...
  const handler = (action, mwApi) => {
    ...
    if (isPendingThunk(action)) {
      ...
      ;(_b = lifecycleMap[requestId]) == null
        ? void 0
        : _b.reject({
...
function buildHooks({
  ...
  function queryStatePreSelector(currentState, lastResult, queryArgs) {
    ...
    const isSuccess =
      ...
      (hasData &&
        ((isFetching && !(lastResult == null ? void 0 : lastResult.isError)) ||
          currentState.isUninitialized))
    return {
      ...currentState,
      data,
        ...
        if ((_a = promiseRef.current) == null ? void 0 : _a.queryCacheKey) {
          dispatch(
            api.internalActions.removeQueryResult({
              queryCacheKey:
                (_b = promiseRef.current) == null ? void 0 : _b.queryCacheKey,
            })
          )
        }
      }, [dispatch])
